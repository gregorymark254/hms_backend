...
from .models import get_current_user, require_admin
...
def get_users(db: Session = Depends(get_db), pagination: Paginator = Depends()):
    query = db.query(models.User)
    total = query.count()
    users = query.offset(pagination.offset).limit(pagination.limit).all()
    count = len(users)
    return Pagination(items=users, offset=pagination.offset, limit=pagination.limit, total=total, count=count)


@router.post('/', response_model=schemas.Users, dependencies=[Depends(get_current_user)])
def add_user(user: schemas.AddUser, db: Session = Depends(get_db)):
